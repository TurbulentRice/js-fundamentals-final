...
export default function QuotelistCard ({ quote, profile, removeFromQuotelist }) {
	const {regularMarketChange, marketChangePercent, currency, down} = unpackQuote(quote);

	return (
		<div id={quote.symbol + "QuoteComponent"} data-symbol={quote.symbol} class="jumbotron quoteDisplayComponent pt-3 pb-2 pl-2">
			<div class="d-flex justify-content-between pl-3">
				...
				<h6 class={(!down ? "gain" : "loss") + " mt-4"}>{!down ? "+" : ''}{currency}{regularMarketChange}</h6>
				<h6 class={(!down ? "gain" : "loss") + " mt-4"}>{!down ? "+" : ''}{marketChangePercent}%</h6>
			</div>
			...
			<div class="d-flex justify-content-between pl-3">
				...
				<button
					...
					type="button"
					class="btn btn-sm text-center removeQuoteComponentBtn"
					data-symbol={quote.symbol}
					onClick={() => removeFromQuotelist(quote.symbol)}>
			...
			<div class="d-flex justify-content-between mt-0 pt-0 pl-3">
				...
				<p>
					<button
						title={quote.symbol + " company info"}
						type="button"
						id={quote.symbol + "InfoBtn"}
						class="btn btn-link btn-sm p-0 mb-0"
						data-symbol={quote.symbol}
						data-placement="left"
						data-content={profile.longBusinessSummary || "n/a"}
						data-toggle="popover"
						data-trigger="focus">
						Company info
					</button>
			...
			<div id={quote.symbol + "ChartDiv"} class="mt-1" style="display:none; height: 360px; width: 100%" data-symbol={quote.symbol}></div>
		</div>
	);
}
